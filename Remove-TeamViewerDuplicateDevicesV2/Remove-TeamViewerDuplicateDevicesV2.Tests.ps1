...
BeforeAll {
    ...
    . "$PSScriptRoot\Remove-TeamViewerDuplicateDevicesV2.ps1" -ApiToken $testApiToken -ExpiryDate (Get-Date) -InformationAction SilentlyContinue
...
Describe 'Remove-TeamViewerDuplicateDevicesV2' {
    ...
    It 'Should list devices that are offline since the expiration date' {
        $result = (Remove-TeamViewerDuplicateDevicesV2 ([DateTime]'2018-12-18') -force:$false -WhatIf)
    ...
    It 'Should remove devices that are offline since the expiration date' {
        $result = (Remove-TeamViewerDuplicateDevicesV2 ([DateTime]'2018-12-18') -force:$true)
        $result | Should -HaveCount 3
        $result[0].DeviceId | Should -Be 'device1'
